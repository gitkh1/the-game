...
## Как запускать в режиме разработки?

1. Убедитесь что у вас установлен `node` и `docker`
2. Выполните команду `yarn bootstrap` - это обязательный шаг, без него ничего работать не будет :)
3. Скопировать содержимое файла `.env.sample` в файл `.env`
4. Выполните команду `yarn setup-hosts` - для пользователей windows запустить терминал от имени администратора
5. Выполните команду `yarn run database:start` чтобы запустить только базу данных
6. Выполните команду `yarn dev --scope=server` чтобы запустить только server
